...
runBATHYs = {'barred1D','terraced1D'};%'planar1D'
figDIR    = '/storage/cms/grimesdj_lab/grimesdj/git/funwave/cases/ripchannel/testCases/test1Dbar/figures/';
%
...
for jj=1:Ndirs
    ...
    tmp    = sum(tmp,3)/Navg;
    BrkSrcX_bp = reshape(tmp,[Ny,Nx,Na]);
    %
...
% $$$     DxSxx_on_H = DxSxx_avg./max(H,0.1);
    ...
    %
    % 5.2) estimate the cumulative breaking force... find where it exceeds 0.90 of total, define as "surfzone edge" or break point
    % 4.1) estimate Lsz as location where cumulative integral of rms(Fbr) is 90.0% of the total
    tmp       = BrkSrcX;
    rmsF      = rms(tmp,[1 3],'omitnan');% this is from (3) above
    rmsF(dry) = nan;
    rmsFtot = sum   (rmsF,'omitnan');
    rmsFcum = cumsum(rmsF,'omitnan');
    rmsFfrac= rmsFcum./rmsFtot;
    % breakpoint at 0.9;
    iBP     = find(rmsFfrac>0.99,1,'first');
    xBP     = x(iBP);
    %
    % and shoreline location
    [~,iSL] = min( abs( mean(eta,3,'omitnan')+h-0.1 ), [], 2);
    xSL = x(iSL);
    %
    % 6) log time-averaged quantities for archiving
    run1D(jj).grid  = runBATHY;    
    run1D(jj).runID = runNAME;
    run1D(jj).x = x;
    run1D(jj).H = H;
    run1D(jj).Hs = Hs;    
    run1D(jj).BrkSrcX = BrkSrcX_avg;
    run1D(jj).BrkDissX= BrkDissX_avg;
    run1D(jj).DxSxx   = DxSxx_avg;
    run1D(jj).Sxx     = Sxx_avg;    
    run1D(jj).PgrdX   = PgrdX_avg;
    run1D(jj).FRCX    = FRCX_avg;
    run1D(jj).DxUUH   = DxUUH_avg;
    run1D(jj).xSL     = xSL;
    run1D(jj).xBP     = xBP;
    %
    fprintf('\nThe break-point location for run: %s-%s, is x_{bp}=%3.1f m\nAnd the surfzone width is W_{sz}=%3.1f m\n',runBATHY,runNAME,xBP,(xBP-xSL))
end
%
