...
runBATHY = 'TINTVMEAN';%'planar1D'
figDIR = '/storage/cms/grimesdj_lab/grimesdj/git/funwave/cases/ripchannel/testCases/TINTVMEAN/figures/';
%
...
Ns = 5500;
%
% Need to estimate this for each run
% T_INTV_mean = 100;
%
...
for jj=1:Ndirs
    ...
    %
    % different wave-averaged time period for each simulation
    T_INTV_mean   = info.T_INTV_mean;
    Navg          = T_INTV_mean;
    %
    ...
    lg1 = legend({'$\partial\eta/\partial x$','$\partial S_{xx}/\partial x$','$\tau_b$','$\partial (u^2 H)/\partial x$','$-\bar{F}_{\mathrm{br}}$'},'NumColumns',2);
