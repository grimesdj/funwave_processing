function [Hs_x,x,eta_bar,h_bar,sig_eta,freqs,Snn_xy,Snn_x,Snn_wg,xsl,xsz,Lsz] = calculate_funwave_wave_height_statistics(rootMat,rootName,bathyFile,Hs,Tp,subDomain,plotter,iwg);
%
...
if h(1,1)<h(1,end)
    ishoreline = find(h_bar+eta_bar>=0.1,1,'last');
else
...
eta_t_xy = dum2(inds,:);
[Snn,freqs] = mywelch(eta_t_xy(inds,:),dt,chnk,0.0);
df      = freqs(2)-freqs(1);
fnquist = 0.5/dt;
nf      = size(Snn,1);
%
