function info = estimate_FUNWAVE_run_statistics_NuBrk(info);
%
% usage: info = estimate_FUNWAVE_run_statistics_NuBrk(info);
%
...
for ii=1:Nf
    ...
    %
    [~,~,dO] = gradientDG(omega);
    dOavg = reshape(dO(:,:,1:N*Ns),ny,nx,Ns,N);
    ...
    cIavg = reshape(cI(:,:,1:N*Ns),ny,nx,Ns,N);
...
nccreate  (info.waveForceFile,'t','Dimensions',{"t",length(x)},'Format','netcdf4')
...
%
dim_xblb = {"Xb",length(Xbins),"Lb",length(Lbins)};
nccreate  (info.waveForceFile,'Lc_pdf','Dimensions',dim_xblb,'Format','netcdf4')
...
ncwriteatt(info.waveForceFile,'Lc','cross-shore bin averaged crest length');
%
nccreate  (info.waveForceFile,'Nc','Dimensions',dim_xb,'Format','netcdf4')
ncwrite   (info.waveForceFile,'Nc',Nx);
ncwriteatt(info.waveForceFile,'Nc','cross-shore bin averaged number of crests');
%
nccreate  (info.waveForceFile,'Lc_plus_std','Dimensions',dim_xb,'Format','netcdf4')
ncwrite   (info.waveForceFile,'Lc_plus_std',sLx);
ncwriteatt(info.waveForceFile,'Lc_plus_std','cross-shore bin averaged crest length plus one standard deviation');
%
...
nccreate  (info.rotVelFile,'t','Dimensions',{"t",length(x)},'Format','netcdf4')
...
%
dim_fx = {"frq",length(freqs),"x",length(x)};
nccreate  (info.waveStatsFile,'Snn','Dimensions',dim_fx,'Format','netcdf4')
ncwrite   (info.waveStatsFile,'Snn',Snn_xy);
ncwriteatt(info.waveStatsFile,'Snn','Description','waterlevel spectra versus cross-shore coordinate');
%
nccreate  (info.waveStatsFile,'frq','Dimensions',{"frq",length(frq)},'Format','netcdf4')
