function flog = convert_funwave_output_to_single_NetCDF(rin,rout,vars,t0,dt0,dx,spanx,rngx,dy,spany,rngy,rmfiles,Nt,tlimits,isBINARY,Mglob,Nglob)
%
...
for jj = 1:nv
    ...
    for kk = 1:nf
        ...
        if (kk-iter)==min(Nt)
            ...
            fout = sprintf([rout,'_%02d.nc'],Nk);
            fprintf(' archiving (kk-iter)=min(nt,Nt): %s \n',fout)
            ...
            ncid=-1;
            try
                ncid = netcdf.open(fout,'nowrite');
                varid= netcdf.inqVarID(ncid,"x");
                if ncid~=-1; netcdf.close(ncid); end
            catch
                if ncid~=-1; netcdf.close(ncid); end
