function info = estimate_FUNWAVE_rotational_impulse(info)
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% need to do this within a loop %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Fx = force.Fx.*info.mask;
Fy = force.Fy.*info.mask;
% 
Ix = sum(Fx,3,'omitnan')*dt;
Iy = sum(Fy,3,'omitnan')*dt;
%
% estimate the rotational/vorticity forcing associated with impulse
[curlI0,cav] = curl(xx,yy,Ix,Iy);
%
% along/cross-shore average to de-alias the under-resolved propagation
Nfx = round(info.dt*sqrt(info.Hs*9.8)/dx); if ~mod(Nfx,2), Nfx = Nfx+1; end
...
cbpos = [xm ym+2*ph+2*ag 0.25*pw ag/1.5];
...
imagesc(f0ax1,y',xp',curlI'), caxis(f0ax1,clims), colormap(f0ax1,cm),
...
imagesc(f0ax2,y',xp',dVORT'), caxis(f0ax2,clims), colormap(f0ax2,cm),
hold(f0ax2,'on'), plot(f0ax2,y,xsl-mean(xsl),'--k')
ylabel(f0ax2,'$x$ [m]','interpreter','latex')
xlabel(f0ax2,'$y$ [m]','interpreter','latex')
text(f0ax2,1250,145,'b) $\Delta \omega_z$','interpreter','latex','fontsize',12)
set(f0ax2,'tickdir','out','ticklabelinterpreter','latex','ylim',-25+[0 200],'ydir','normal','xdir','reverse')
% make colorbar
f0cb = axes('units','centimeters','position',cbpos);
imagesc(f0cb,clrs,0,reshape(cm,1,256,3))
ylabel(f0cb,'~[1/s]','interpreter','latex','fontsize',12,'rotation',0,'horizontalalignment','left','verticalalignment','bottom')
...
% save figure
exportgraphics(fig0,[info.rootSim,filesep,'figures',filesep,info.rootName,'rotational_impulse_and_vorticity_change.pdf'])
%
%
...
% $$$ [dVORT_spec,ky] = alongshore_spectra_estimate(info,dVORT_yavg);
% estimate y-rms
curlI_yrms = rms(curlI_yavg,1,'omitnan');
dVORT_yrms = rms(dVORT_yavg,1,'omitnan');
% estimate y-avg
curlI_yavg = nanmean(curlI_yavg,1);
dVORT_yavg = nanmean(dVORT_yavg,1);
%
% 0) create file for saving stats
impulse = matfile([info.rootMat,info.rootName,'rotational_impulse.mat'],'writable',true);
impulse.ky = ky;
impulse.curlI = curlI;
impulse.dVORT = dVORT;
impulse.curlI_spec = curlI_spec;
impulse.dVORT_spec = dVORT_spec;
impulse.curlI_dVORT_cospec = curlI_dVORT_cospec;
impulse.xp    = xp;
impulse.yp    = y;
info.rotImpulseFile = impulse.Properties.Source;
save(info.fileName,'-struct','info')
%
% start plots, binning the spectra into five cross-shore bins
...
plot(f1ax1,xp,dVORT_yrms,'-k',xp,curlI_yrms,'-r','linewidth',2)
xlabel(f1ax1,' $(x-\bar{x}_\mathrm{sl})$~[m]','interpreter','latex')
ylabel(f1ax1,'$\mathrm{rms}_y(\cdot)$~[1/s]','interpreter','latex')
...
curlI_spec_binned = nan(length(ky),Nb);
dVORT_spec_binned = nan(length(ky),Nb);
cospec_binned = nan(length(ky),Nb);
for ii = 1:Nb
    inbin  =  find( xp>=(xbins(ii)-db/2) & xp<(xbins(ii)+db/2));
    tmp0    = nanmean(curlI_spec(:,inbin),2);
    tmp1    = nanmean(dVORT_spec(:,inbin),2);
    tmp2    = nanmean(curlI_dVORT_cospec(:,inbin),2);
    loglog(f1ax2,ky,sqrt(tmp1*(ky(2)-ky(1))),'-',ky,sqrt(tmp0*(ky(2)-ky(1))),'--','color',cm2(ii,:),'linewidth',2),
    curlI_spec_binned(:,ii)=tmp0;
    dVORT_spec_binned(:,ii)=tmp1;
    cospec_binned(:,ii)=tmp2;
end
% also average in wavenumber space
Nf   = 15;
ff   = hamming(Nf)./hamming(Nf);
curlI_spec_binned = conv2(curlI_spec_binned,ff,'same');
dVORT_spec_binned = conv2(dVORT_spec_binned,ff,'same');
cospec_binned     = conv2(cospec_binned,ff,'same');
coh = abs(cospec_binned)./sqrt(curlI_spec_binned.*dVORT_spec_binned);
%
f1ax2.XAxis.Scale='log';
xlabel(f1ax2,' $k_y$~[m$^{-1}$]','interpreter','latex')
ylabel(f1ax2,' [1/s]','interpreter','latex')
grid(f1ax2,'on')
...
exportgraphics(fig1,[info.rootSim,filesep,'figures',filesep,info.rootName,'rotational_impulse_alongshore_stats_vs_x.pdf'])
%
%
%
% start plots, binning the spectra into five cross-shore bins
xm = 2; ym = 2; pw = 10; ph = 3.5; ag=0.25; ps = [1.5*xm+pw, 2*ym+ag+1.5*ph];
ppos1 = [xm        ym             pw       1.5*ph];
cbpos = [xm+0.6*pw ym+1.25*ph      0.3*pw   ag];
...
%
for ii = Nb:-1:1
    loglog(f2ax2,ky,coh(:,ii),'-','color',cm2(ii,:),'linewidth',2),
end
%
yline(f2ax2,1-(0.05).^(1/(14)),'--r','linewidth',2)
...
imagesc(f2cb,xbins,0,reshape(cm2,1,Nb,3))
xlabel(f2cb,'$(x-\bar{x}_\mathrm{sl})$','interpreter','latex','rotation',0,'fontsize',12)
set(f2cb, 'tickdir','out','ticklabelinterpreter','latex','yaxislocation','right','xaxislocation','bottom','ytick',[],'ticklength',2*get(f2cb,'ticklength'),'xtick',xbins)
exportgraphics(fig2,[info.rootSim,filesep,'figures',filesep,info.rootName,'rotational_impulse_coherence.pdf'])
%
return
%% below is copied code from ...rotational_power_input.m
%
% 1) estimate Prot = Urot * Fbr
I  = force.Fx.*rot_vel.Urot + force.Fy.*rot_vel.Vrot;
% $$$ Prot = power.Prot;
%
